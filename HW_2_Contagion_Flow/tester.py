...
# You should put tester.py in the same folder as hw2_p8.py and hw2_p9.py, and run this file.
...
# Problem 8 sanity check
testGraph = hw2_p8.UndirectedGraph(3)
testGraph.add_edge(0, 1)
testGraph.add_edge(1, 2)
infected = hw2_p8.contagion_brd(testGraph, [0], 0.1)
assert len(infected) == 3
infected = hw2_p8.contagion_brd(testGraph, [0], 0.49)
assert len(infected) == 3
infected = hw2_p8.contagion_brd(testGraph, [0], 0.50)
assert len(infected) == 3
infected = hw2_p8.contagion_brd(testGraph, [0], 0.51)
assert len(infected) == 1
infected = hw2_p8.contagion_brd(testGraph, [0], 0.7)
assert len(infected) == 1

testFbGraph = hw2_p8.create_fb_graph("facebook_combined.txt")
assert testFbGraph.number_of_nodes() == 4039
assert testFbGraph.check_edge(107, 1453) == True
infected = hw2_p8.contagion_brd(testFbGraph, [0], 0)
assert len(infected) == 4039
infected = hw2_p8.contagion_brd(testFbGraph, [0], 1)
assert len(infected) == 15

print("Problem 8 sanity check passed")

# Problem 9 sanity check
testGraph = hw2_p9.WeightedDirectedGraph(3)
...
# 3 drivers, 2 riders
C = [[0, 0], [1, 0], [0, 1]]
result = hw2_p9.max_matching(3, 2, C)
assert len(result) == 3
assert result == [None, 0, 1]

C = hw2_p9.random_driver_rider_bipartite_graph(2, 1)
assert C == [[1, 1],[1,1]]
